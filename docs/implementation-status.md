...
2026-09-21，本轮 P0/P1 修正已合入共享运行时、三个适配器与知识仓库。macOS 已有原生使用、实际 OS 定时投递、知识撤下和反馈自动提 PR 的证据；最终业务经验的发布、Bot 审阅合入与新任务使用仍在推进。本文不作为首版全部验收通过的结论。Windows 仍在首版支持目标内，按用户安排在合入后使用专门机器验收。
