...
remote-dev 0.9.5 `28213bf3`，对应 CI 全部通过。三端适配器已固定这些正式提交并
合入：Codex [PR10](https://github.com/mindie-agent/mindie-agent-codex/pull/10)
`6146d862`、Kimi [PR8](https://github.com/mindie-agent/mindie-agent-kimi/pull/8)
`d258d144`、Claude Code [PR8](https://github.com/mindie-agent/mindie-agent-cc/pull/8)
`c909363f`；准确 head 的 CI 均通过。
三端取消调度已验证真实 macOS 成功、缺失及系统失败路径；
独立上报器已验证剩余时间不足不变更、真实版本交接、失败回滚、同目标失败抑制，
以及停止或关闭后不复活。三端原生上报入口与持久安装路径按正式旧基线单独记录，
不能据此宣称最终候选已安装。新经验的实际整理暴露了摘要扩大结论和公开软件版本
误脱敏，脱敏修正已合入，整理提示同步随适配器发布。此前 Kimi 原生业务因验收
控制器的总截止及审批处理迟延而未产生业务 Stop；完整贡献验收尚未通过。
新的 Kimi K3/max 已按用户授权开启贡献，实际读取任务书并开始算子源码调查；
采用原生自动审批及半天后进度检查，不设置统一业务截止，也不重放旧失败采集
或手工补造经验。Codex 生产定时器已自动更新到上述正式提交，安装与精确依赖
回读通过，Stop 仍为 trusted；Kimi/CC 最终包在独立 profile 实际安装回读通过。
CC 两个 MCP 原生健康检查 Connected，不据此宣称新增模型或贡献验收。
准确结果、失败保留及剩余范围见[本轮验收](release-acceptance-2026-09-23.md)，
设计边界见[首版使用流程收敛](release-readiness-2026-09-22.md)。下文保留历史基线。
...
用户审阅信任后，原生回读确认同一 Stop Hook 为 `trusted`，超时仍为 2 秒。
随后一次实际 `gpt-5.6-luna` / `max` 任务只返回验收标记；宿主记录一组
`hook/started`、`hook/completed`，Stop 在 57 毫秒完成，未重复运行，模型工具调用为零。
自有进程组已退出，知识贡献未开启，生产故障上传只读回查仍为未配置。
此项证明当前安装版本在贡献关闭时的原生 Stop 交付，不替代贡献开启后的整理发布链路。

本次全局配置逐字未变的检查未通过，原始结果保留为 false；验收目录存在一条项目信任
记录，随后仅清理该自有目录条目，保留用户的 Hook 信任及其他配置语义。因原控制器未
保存完整运行前配置，不宣称已证明所有原始字节差异的来源。这不混入 Stop 交付的通过
结论。本地原生事件、核对及清理记录位于 `acceptance/codex-native-stop-final/`。
