...
Codex 生产原有 OS 定时器已自然更新到 `6146d862`；第一次人工 check 仅命中
既有检查间隔，没有尝试安装，随后未重发 check 或修改节流。原生 inventory
回读版本 `0.1.0+codex.20260923022232128416`、installed/enabled 均为 true，
实际解释器的三项共享依赖与上述正式 SHA 一致。原生 Stop Hook 仍为 trusted、
enabled，超时 2 秒，使用未改变的保留入口。没有新建任务或模型请求，不将
安装和 Hook 配置回读称为本轮新增 Stop 执行或已有任务 MCP 连接证据。

Claude Code 在独立 profile 安装正式来源后，经保留 launcher 的一次正常 check
切换到 `c909363f`，原生 plugin list、实际 `mcp list` 的两个 Connected 结果及
三项精确 VCS 依赖回读均通过。没有新模型请求，没有安装到用户默认 profile，
默认配置文件未变。首次 setup 已成功，但验收脚本错误地把无 Git 元数据的官方
archive 当作 checkout，要求其内容指纹必须等于 commit SHA；该断言失败保留。
后续复用已安装状态完成正常更新，未重跑 setup 或重建 profile；不是安装失败后
覆盖证据。MCP 连接检查不代替模型工具调用、Hook 交付或贡献内容验收。

Kimi 正式 `d258d144` 已在独立原生 profile 完成实际插件安装及三项精确 VCS
依赖回读，全局配置未变。新的 K3/max 任务已经完成原生显式启用和贡献开启，
读取完整业务任务书并开始检查源码；native wire 确认 permission mode 为 auto、
实际请求 effort 为 max，没有待审批。该任务用于用户授权的实际算子开发，
允许自主修复开发及插件问题，不设置整项业务截止；半天后检查进展。
这是已启动的业务工作，不是算子、经验质量或贡献闭环验收通过。原生 Stop、
自动整理、发布和新任务复用仍按实际发生分别判断。

共享上报器候选交接及失败恢复、三端正式基线的持久安装路径已通过上述真实 macOS
检查；三端最终提交的实际安装回读也已完成，具体 profile 和结论范围如上。
新知识贡献使用上述仍在运行的真实工作；
旧失败材料不重放。
后续结果在本文追加准确版本与限制。
